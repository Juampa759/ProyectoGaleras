...
class rpt_reporte {
	...
	public function __construct($id_rpt, $des_rpt, $sql_rpt, $est_rpt,$id_men,$tipo_rpt){
		$this->id_rpt = $id_rpt;
		$this->des_rpt = $des_rpt;
		$this->sql_rpt = $sql_rpt;
		$this->est_rpt = $est_rpt;
		$this->id_men = $id_men;
		$this->tipo_rpt = $tipo_rpt;
	}
	...
	public static function recuperar(BdConex $con, $id_rpt){
		...
		try{
			...
			$id_men = $fila['id_men'];
			$tipo_rpt = $fila['tipo_rpt'];
			return new rpt_reporte($id_rpt, $des_rpt, $sql_rpt, $est_rpt, $id_men, $tipo_rpt);
		}catch(Exception $e){
			throw($e);
			return NULL;
		}
	...
	public function __get($var){
		switch($var){
			...
			case('tipo_rpt'): return ($this->tipo_rpt); break;
		}
	}
	public function __set($var,$val){
		switch($var){
			...
			case('tipo_rpt'): $this->tipo_rpt = $val; break;
		}
	}
	...
	public static function getFilteredBy(BdConex $con, $est_rpt='%' ){
		...
		try{
			...
			for($i=0;$fila = $con->fetch($res);$i++){
				...
				$id_men = $fila['id_men'];
				$tipo_rpt = $fila['tipo_rpt '];
				$arr[$i]=new rpt_reporte($id_rpt, $des_rpt, $sql_rpt, $est_rpt ,$id_men, $tipo_rpt);
			}
			return $arr;
		}catch(Exception $e){
			throw($e);
